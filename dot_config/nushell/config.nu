...
# $env.config.hooks.env_change = {}  # Removed: was overwriting zoxide's PWD hook
...
# USGC PROMPT (translated from bash)
# Original: PS1="\[$(tput setaf 7)\]❬\h❭ \[$(tput setaf 2)\]\W\[$(tput setaf 1)\] ●\[$(tput sgr0)\] "
$env.PROMPT_COMMAND = {||
  let hostname = (hostname | str trim)
  let dir = ($env.PWD | path basename)
  $"(ansi white)❬($hostname)❭ (ansi green)($dir)(ansi red) ●(ansi reset) "
}
...
$env.PROMPT_COMMAND_RIGHT = {||
  ...
  if ($jj_out | is-not-empty) {
    $jj_out
  } else {
    # Fall back to git
    try {
      let branch = git symbolic-ref --short HEAD err> $null_device | str trim
      let status = git status --porcelain err> $null_device | str trim
      let dirty = if ($status | is-not-empty) { $"(ansi yellow_bold)*" } else { "" }
