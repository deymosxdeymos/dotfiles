...
ClickableIcon {
	...
	readonly property bool connected: {
		const a = adapter;
		if (!a || !a.devices) return false;
		return a.devices.values.some(device => device.connected);
	}
	...
	onPressed: event => {
		event.accepted = true;
		if (event.button === Qt.RightButton && hasAdapter) {
			showMenu = !showMenu;
		}
	}

	onClicked: event => {
		if (event.button === Qt.LeftButton && hasAdapter) {
	...
	image: {
		const a = adapter;
		if (!a) return "root:/icons/bluetooth-slash.svg";
		return a.enabled
	...
	property var tooltip: TooltipItem {
		...
		owner: root
			...
			Label {
				text: root.hasAdapter ? "Bluetooth" : "Bluetooth unavailable"
			}
	}
	...
	property var rightclickMenu: TooltipItem {
		...
		isMenu: true
		show: root.hasAdapter && root.showMenu
		onClose: root.showMenu = false

		Loader {
			width: 400
			active: (root.showMenu || rightclickMenu.visible) && root.hasAdapter
