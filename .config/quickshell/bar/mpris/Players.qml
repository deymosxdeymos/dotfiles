...
FullwidthMouseArea {
	...
	onWheel: event => {
		...
		if (!player || !MprisController.canChangeVolume) return;
		player.volume = Math.max(0, Math.min(1, player.volume + (event.angleDelta.y / 120) * 0.05));
	}
	...
	Item {
		...
		BackgroundArt {
			...
			function updateArt(reverse: bool) {
				const artUrl = MprisController.activeTrack?.artUrl ?? "";
				console.log("update art", artUrl)
				this.setArt(artUrl, reverse, false)
			}
		...
		ColumnLayout {
			...
			FrameAnimation {
				...
				onTriggered: {
					if (positionInfo.player) positionInfo.player.positionChanged();
				}
			}
			...
			Item {
				...
				Rectangle {
					...
					Rectangle {
						...
						width: {
							const player = root.activePlayer;
							if (!player || !player.length) return 0;
							return parent.width * (player.position / player.length);
						}
					}
				}
